...
Options:
  ...
  --mode MODE          package, deps, or probe. deps builds the cached
                       ...
                       Default: package
  --rpm-av1 MODE       RPM AV1 bcond: auto, on, off. Default: auto.
...
Examples:
  ...
  packaging/scripts/test_distro_package_builds.sh --rpm-av1 on --image opensuse/tumbleweed:latest
EOF
}
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		--rpm-av1)
			if [[ $# -lt 2 ]]; then
				echo "--rpm-av1 requires an argument" >&2
				exit 2
			fi
			RPM_AV1=$2
			shift 2
...
case "$RPM_AV1" in
	auto | on | off)
		;;
	*)
		echo "--rpm-av1 must be one of: auto, on, off" >&2
		exit 2
		;;
esac
